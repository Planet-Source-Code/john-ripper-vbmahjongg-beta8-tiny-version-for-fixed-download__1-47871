...
Private Function drawMnuBitmap(ByVal hwnd As Long, ByVal drawInfoPtr As Long) As Long
    ...
    If (aDIS.itemState And ODS_CHECKED) = ODS_CHECKED Then
        Set lPic = LoadPicture(App.Path & "\check.bmp") 'FrmSprites.imgCheck.Picture 'LoadResPicture("checked", vbResBitmap)
        
        Set lMask = LoadPicture(App.Path & "\checkmask.bmp") 'FrmSprites.imgCheckMask.Picture 'LoadResPicture("checked", vbResBitmap)
